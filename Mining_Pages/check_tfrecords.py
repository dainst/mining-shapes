...
dataset= Dataset.import_from('/home/images/OUTPUT/Lattara6_pages.tfrecord', 'tf_detection_api')
datasetimages = dataset.select(lambda item: item.has_image)
dataset.export('/home/images/OUTPUT/Lattara6_pages_reimport2', 'tf_detection_api', save_images=True)
label_categories = dataset.categories().get(AnnotationType.label,LabelCategories())
label_ids = OrderedDict((label.name, 0 + idx) for idx, label in enumerate(label_categories.items))
#get_label = lambda label_id: label_categories.items[1].name if label_id is not None else ''
print(label_ids)
print(label_categories.items[1].name)
