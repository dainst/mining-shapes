...
def run_inference_for_single_image(image, graph):
    ...
    if 'detection_masks' in tensor_dict:
        ...
        detection_masks_reframed = ops.utils_ops.reframe_box_masks_to_image_masks(
...
def extract_detections_figureid(df, figid_category_index):
    figid_detections = pd.DataFrame(
        df['figid_detections'].tolist()).reindex(df.index)
    detections = figid_detections.applymap(lambda x: x[0] if type(
        x) == np.ndarray else x).reindex(figid_detections.index)
    detections = detections.merge(figid_category_index, on=[
                                  'figid_detection_classes'], how='left')
...
figid_detections = figures_step1.apply(
    extract_detections_figureidv2, figid_category_index=figid_category_index, axis=1)
